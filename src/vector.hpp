...
namespace smallpt {
	...
	public:
		...
		[[nodiscard]]
		constexpr std::size_t MinDimension() const noexcept {
			return (m_x < m_y && m_x < m_z) ? 0u : ((m_y < m_z) ? 1u : 2u);
		}
		
		[[nodiscard]]
		constexpr std::size_t MaxDimension() const noexcept {
			return (m_x > m_y && m_x > m_z) ? 0u : ((m_y > m_z) ? 1u : 2u);
		}
	...
	constexpr Vector3 Clamp(const Vector3& v,
		...
		return {
			std::clamp(v.m_x, low, high),
			std::clamp(v.m_y, low, high),
			std::clamp(v.m_z, low, high) }
		;
	}
	[[nodiscard]]
